// This playground is purely generated by AI to help on development, should really refactor
...
const DevPlayground = () => {
  ...
  useEffect(() => {
    if (connected && wallet) {
      ...
      // Use your configured Blockfrost provider for fetching UTxOs and other blockchain data
      const fetcher = BlockFrostProd(); // This is your properly configured Blockfrost provider
  ...
  const submitTransaction = async (hex: string) => {
    ...
    try {
      ...
      // Clear the current transaction hex since it's been submitted
      setTxHex('');
      ...
      // Optionally reload UTxOs to show the updated state
      if (showUtxosBrowser) {
  ...
  const lookupTxUtxos = async () => {
    ...
    try {
      ...
      // Use the contract's built-in getUtxobyHash method to get UTxOs for the transaction
      const utxos = await contract.getUtxobyHash(txHashInput.trim());

      // Convert single UTxO to array format for consistency with the UI
  ...
  // Utility function: Hash BPMN content
  const handleHashBpmn = async () => {
    try {
      ...
      // Hash the BPMN content using Mesh's resolveDataHash
  ...
  // Utility function: Sign transaction hex
  ...
  const handleRunTask = async () => {
    ...
    try {
      // First try to find the UTxO in wallet's UTxOs
      const allUtxos = await wallet.getUtxos();
      let utxo = allUtxos.find(
        (u) =>
          u.input.txHash === runTaskForm.utxoHash &&
          u.input.outputIndex.toString() === runTaskForm.utxoIndex
      );

      // If not found in wallet, try fetching it from Blockfrost
      if (!utxo) {
        try {
          utxo = await contract.getUtxobyHash(
            runTaskForm.utxoHash,
            parseInt(runTaskForm.utxoIndex)
          );
  ...
  const handleCompensated = async () => {
    ...
    try {
      // Use getUtxobyHash to find the UTxO automatically
      console.log('Fetching UTxO for hash:', compensatedForm.utxoHash);
  ...
  const handleUncompensated = async () => {
    ...
    try {
      ...
      let utxo = allUtxos.find(
        (u) =>
          u.input.txHash === uncompensatedForm.utxoHash &&
          u.input.outputIndex.toString() === uncompensatedForm.utxoIndex
      );
      ...
      if (!utxo) {
        try {
          utxo = await contract.getUtxobyHash(
            ...
            parseInt(uncompensatedForm.utxoIndex)
          );
        } catch (blockfrostError) {
          console.warn(
            'Blockfrost fetch failed, continuing with wallet-only approach:',
